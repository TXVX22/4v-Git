...
void DirectxFunctions::RenderDirectX()
{
	...
	if (GetForegroundWindow() == Target.Window)
	{
		...
		if (nametp)
		{
			//// Adresse der X-Koordinate
			//unsigned long xAddr = 0x04606094;

			//// Pointer zu den Adressen casten
			//float* xPtr = reinterpret_cast<float*>(xAddr);
			//float* yPtr = xPtr + 2;  // Adresse der Y-Koordinate ist xAddr + 8
			//float* zPtr = xPtr + 1;  // Adresse der Z-Koordinate ist xAddr + 4

			//// hier hab ich die vec 3 koords zu einer variable gemacht
			//Vector3 vector3 = { *xPtr, *yPtr, *zPtr };

			for (int i = 0; i < 255; i++)
			{
				if (ents[i] != 0)
				{
					ents[i]->coords = LP->coords;
					/*ents[i]->coords.x = vector3.x;
					ents[i]->coords.y = vector3.z;
					ents[i]->coords.z = vector3.y;*/
				}
			}
		}
		...
		if (bMenuEnabled)
		{
			...
			DirectX.Font->DrawTextW(NULL, L"4Vision Hack", -1, &pos, 0, D3DCOLOR_ARGB(255, 5, 5, 5));
			pos.top += 20;
...
{
	// entityhook1
	DWORD entityAddy = FindPattern("TClient.exe", "\x89\x86\x84\x00\x00\x00\x8B\x46\x48\x89\x86\x88\x00\x00\x00\x8B\x46\x4C\x89\x86\x8C\x00\x00\x00\x8B\x45\x0C", "xxxxxxxxxxxxxxxxxxxxxxxxxxx");
	// MsgBoxAddy(entityAddy);
	EntlistJmpBack = entityAddy + 0x6;
	PlaceJMP((BYTE*)entityAddy, (DWORD)entityhook, 6);

	// 
	DWORD Playeraddy = FindPattern("TClient.exe", "\x89\x81\x38\x0B\x00\x00\x8B\x4D\xFC\x33\xCD\x5E", "xxxxxxxxxxxx");
	// MsgBoxAddy(entityAddy);
	PlayerJmpBack = Playeraddy + 0x6;
	PlaceJMP((BYTE*)Playeraddy, (DWORD)playerhook, 6);
